...
public class TLView extends View {
	...
	private static final long YEAR_MILLIS = DAY_MILLIS * 365;

	private Paint blackline, redline, magentaline, redtext20, blacktext30,
			blacktext20, blacktext15, backgroundgradient;
	...
	float sec_pixels, min_pixels, hour_pixels, day_pixels, year_pixels;

	/**
	 * reusable calendar class object for rounding time to nearest applicable
	 * unit in onDraw
	 */
	Calendar acalendar;
	...
	public void onWindowFocusChanged(boolean hasWindowFocus) {
		...
		backgroundgradient.setShader(new LinearGradient(0, 0, 0, getHeight(),
				Color.WHITE, 0xFFAAAAAA, Shader.TileMode.CLAMP));
	}
	...
	private void init() {
		...
		left = System.currentTimeMillis() - 100
				* android.text.format.DateUtils.YEAR_IN_MILLIS;
		right = System.currentTimeMillis() + 100
				* android.text.format.DateUtils.YEAR_IN_MILLIS;
		span = right - left;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		year_pixels = (float) YEAR_MILLIS * pixels_per_milli;
		...
		/*
		 * canvas.drawText("Minutes in view: " + Integer.toString((int) (width /
		 * min_pixels)), 0, getHeight()-80, redtext20);
		 * canvas.drawText("Day width: " + Float.toString(day_pixels), 0,
		 * getHeight()-60, redtext20); canvas.drawText("day > width: " +
		 * Boolean.toString(width < day_pixels), 0, getHeight()-40, redtext20);
		 * canvas.drawText("Hour width: " + Float.toString(hour_pixels), 0,
		 * getHeight()-20, redtext20);
		 */

		// round calendar down to leftmost hour
		acalendar.setTimeInMillis(left);
		// floor the calendar to various time units to find where (in ms) they
		// start
		...
		if (hour_pixels > 2) {
			...
			for (long i = next; i < right; i += HOUR_MILLIS) {
				...
				if (hour_pixels < 4) {
					if (h24 == 12)
						canvas.drawLine(x, rulery - 10, x, rulery + 10,
								blackline);
					if (h12 == 6)
					...
					if (h24 == 12) {
						canvas.drawLine(x, rulery - 20, x, rulery + 10,
								blackline);
					...
					if (h24 == 12) {
						canvas.drawLine(x, rulery - 30, x, rulery + 15,
								blackline);
						drawstar((int) x, rulery - 30, 20, 20, canvas,
								blackline);
					} else if (h12 == 6) {
						canvas.drawLine(x, rulery - 20, x, rulery + 10,
								blackline);
						drawstar((int) x, rulery - 20, 15, 15, canvas,
								blackline);
					} else if ((h12 == 3) || (h12 == 9)) {
						canvas.drawLine(x, rulery - 15, x, rulery + 5,
								blackline);
						drawstar((int) x, rulery - 15, 15, 15, canvas,
								blackline);
					} else {
						canvas.drawLine(x, rulery - 10, x, rulery, blackline);
						drawstar((int) x, rulery - 10, 10, 10, canvas,
								blackline);
					}
				...
				/*
				 * for (int j = (int) (x + (float)min_pixels/6f); j < x +
				 * min_pixels && j < right; j += (float)min_pixels/6f) {
		...
		/*
		 * acalendar.set(Calendar.ERA, GregorianCalendar.BC);
		 * acalendar.set(Calendar.YEAR, 2000);
		 */
		...
		do {
			...
			int daysthismonth = acalendar
					.getActualMaximum(Calendar.DAY_OF_MONTH);
			String monthnamelong = acalendar.getDisplayName(Calendar.MONTH,
					Calendar.LONG, Locale.US);
			String monthnameshort = acalendar.getDisplayName(Calendar.MONTH,
			...
			if(acalendar.get(Calendar.ERA) == GregorianCalendar.BC)
				{
				String year = Integer
						.toString(acalendar.get(Calendar.YEAR));
				if (acalendar.get(Calendar.ERA) == GregorianCalendar.BC) {
					year = year.concat("BC");
					
					//x = (int) ((daymsx - right) / span * width);
					//x = width + x;
				}
			...
			if (monthnumber == 0) {
				...
				if (year_pixels > 50) {
					...
					canvas.drawText(year, x + 8, rulery - 167, blacktext30);
				} else if (acalendar.get(Calendar.YEAR) % 100 == 0
						|| (acalendar.get(Calendar.YEAR) % 10 == 0 && year_pixels > 10)
						|| (acalendar.get(Calendar.ERA) == GregorianCalendar.BC && acalendar.get(Calendar.YEAR) % 100 == 0) 
						|| acalendar.get(Calendar.YEAR) == 36) {
			...
			if (day_pixels < 1)
				...
				canvas.drawText(monthnameshort, x + 50, rulery + 30,
						blacktext30);
				canvas.restore();
			...
			for (int date = 1; date <= daysthismonth; date++, daynumber++, daymsx += DAY_MILLIS) {
				...
				if (day_pixels < 10) {
					...
					// tiny days
					if (daynumber == 1)
						canvas.drawLine(x, rulery - 50, x, rulery + 10,
								blackline);
					else
					...
					// big days
					if (daynumber == 1)
						canvas.drawLine(x, rulery - 100, x, rulery + 50,
								blackline);
					else
						canvas.drawLine(x, rulery - 100, x, rulery + 10,
								blackline);

					drawstar(x, rulery - 100, 20, 20, canvas, blackline);
					canvas.drawText(Integer.toString(date), x + 10,
							rulery - 40, blacktext30);
					canvas.drawText(DayShort(daynumber, false), x + 10,
							rulery - 10, blacktext30);
				} else {
					// sideways days
					if (daynumber == 1)
						canvas.drawLine(x, rulery - 60, x, rulery + 30,
								blackline);
					else
					...
					canvas.rotate(-90, x, rulery);
					canvas.drawText(Integer.toString(date), x + 10,
							rulery + 30, blacktext30);
					canvas.drawText(DayShort(daynumber, true), x + 40,
							rulery + 30, blacktext30);
					canvas.restore();
				}
			}
		...
		if (calstuff != null) {
			for (int i = 0; i < calstuff.ourEvents.size(); i++) {
				...
				if ((e.title == null) || (day_pixels < 35))
					...
					canvas.save();
					canvas.clipRect(r);
					canvas.drawText(t, x, y + h / 2, blacktext30);
					canvas.restore();
					...
					if (e.desc != null)
						if (e.desc.length() > 0)
							canvas.drawText(e.desc, x + h / 2, y + 3 * h / 2,
									blacktext15);
				}
			}
		}
	...
	public boolean onTouchEvent(MotionEvent motionevent) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			// panning
			if (fingers == 1) {
				// how far to scroll in milliseconds to match the scroll input
				// in pixels
			...
			// scaling
				...
				if (Math.abs(new1x - new2x) < 10)
					return true;
				if (finger1x > finger2x)
					if (new1x < new2x)
						return true;
				if (finger1x < finger2x)
					if (new1x > new2x)
				...
				// y values are set to the millisecond time shown at the old
				// finger1x and
				// finger2x, using old left and right span
				// construct a new line equation through points
				// (new1x,y1),(new2x,y2)
				// f(x) = y1 + (x - new1x) * (y2 - y1) / (new2x - new1x)
				left = (long) (y1 + (0 - new1x) * (y2 - y1) / (new2x - new1x));
				right = (long) (y1 + (width - new1x) * (y2 - y1)
						/ (new2x - new1x));
		...
		case MotionEvent.ACTION_POINTER_UP:
			...
			if (id == finger1id) {
				// 1st finger went up, make 2nd finger new firstfinger and go
				// back to panning
				finger1id = finger2id;
				finger1x = finger2x; // copy coords so view won't jump to other
										// finger
				finger1y = finger2y;
				fingers = 1; // panning
	...
	private void drawstar(int x, int y, int halfwidth, int halfheight,
